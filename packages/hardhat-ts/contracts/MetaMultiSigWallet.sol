...
contract MetaMultiSigWallet {
  ...
  function addSigner(address newSigner, uint256 newSignatureRequired) public onlySelf {
    require(newSigner != address(0), "addSigner: zero addres");
    require(!isOwner[newSigner], "addSigner: owner not unique");
    require(newSignatureRequired > 0, "addSigner: must be non-zero sigs required");
    isOwner[newSigner] = true;
    signaturesRequired = newSignatureRequired;
    emit Owner(newSigner, isOwner[newSigner]);
  }

  function removeSigner(address oldSigner, uint256 newSignatursRequired) public onlySelf {
    require(isOwner[oldSigner], "emoveSigner: not owner");
    require(newSignatursRequired > 0, "removeSigner: must be non-zero sigs required");
    isOwner[oldSigner] = false;
    signaturesRequired = newSignatursRequired;
    emit Owner(oldSigner, isOwner[oldSigner]);
  }

  function transferFund(address toAddress) public payable onlySelf returns (bool) {
    require(toAddress != address(0), "transferFund: to is address zero");
    (bool isSuccess, ) = toAddress.call{ value: msg.value }("");
    require(isSuccess, "transferFund: tx fail");
    return true;
  }

  function updateSignaturesRequired(uint256 newSignatursRequired) public onlySelf {
    require(newSignatursRequired > 0, "updateSignaturesRequired: must be non-zero sigs required");
    signaturesRequired = newSignatursRequired;
  }
